...
const MealsDisplay = ({isdarkOn, isSearching, setisSearching, inputValue, meals, setmeals}) => {
  ...
  return (
    <div style={{ backgroundColor: isdarkOn?"#04030F":"#fff", color: isdarkOn&&"black"}} className='flex px-4 flex-col items-center justify-center gap-5 py-1'>
       <div className='bg-lime-50 overflow-hidden rounded-xl flex  flex-col justify-center h-70 gap-1'>
        <h1 className='text-[1.2rem]  px-4 pt-4 text-left font-serif' data-aos="fade-out">Explore <span className='text-yellow-300 text-[1.2em]'>Delicious</span> Recipes From All Around The World With NomYumm!</h1>
        ...
        <div className='p-3 text-black font-semibold text-center grid grid-cols-1 gap-10 mt-6'>
           
           {meals.length>0? meals.map(meal=><div className='max-w-60 grid gap-1 rounded-xl border border-none outline-none bg-lime-50  capitalize overflow-hidden' key={meal.idMeal}>
               <div className='text-black rounded'><img className="object-cover" src={meal.strMealThumb}></img></div>
