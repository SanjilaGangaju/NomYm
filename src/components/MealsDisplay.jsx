...
const MealsDisplay = ({isdarkOn, isSearching, setisSearching, inputValue, meals, setmeals}) => {
  ...
  return (
    <div style={{ backgroundColor: isdarkOn?"#04030F":"#fff", color: isdarkOn&&"black"}} className='flex px-4 md:px-15 flex-col items-center justify-center gap-5 py-1'>
       <div className='bg-lime-50 overflow-hidden rounded-xl flex  flex-col md:flex-row md:gap-2  justify-center h-70 gap-1'>
        <h1 className='text-[1.2rem] md:m-auto md:text-center md:text-[1.9rem] md:px-7 px-4 pt-4 text-left font-serif' >Explore <span className='text-yellow-300 text-[1.2em] md:-text-[1.4rem'>Delicious</span> Recipes From All Around The World With NomYumm!</h1>
        ...
        <div className='text-black font-semibold text-center grid grid-cols-1 md:grid-cols-3 md:gap-10 gap-10 mt-6'>
           
           {meals.length>0? meals.map(meal=><div className='w-64 grid gap-1 rounded-xl border border-none outline-none bg-lime-50  capitalize overflow-hidden' key={meal.idMeal}>
            ...
            </div>):<div className="flex items-center m-auto justify-center text-2xl "><p className='text-center text-xl font-semibold opacity-70' style={{color: isdarkOn?"white":"black"}}>Couldnot Find the Dish You are Looking For</p></div>}
        </div>
        </div>
